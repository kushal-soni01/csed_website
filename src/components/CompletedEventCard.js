...
import {
	...
	Briefcase,
	UserRoundCheck,
	UserPlus, // NEW: Icons for speaker types
} from "lucide-react";
...
const CompletedEventCard = ({ event }) => {
	...
	const mainSpeakers =
		event.speakers?.filter((speaker) => speaker.type === "main") || [];
	const assistantSpeakers =
		event.speakers?.filter(
			(speaker) => speaker.type === "assistant" || !speaker.type
		) || []; // Catch those without a 'type' as assistants/support
			...
			<div className="event-card" onClick={openModal}>
				...
				<div className="event-card-content">
					...
					<p className="event-date">
						<CalendarDays size={16} className="card-icon" />{" "}
						{event.date}
					</p>
					<p className="event-summary">
						<Lightbulb size={16} className="card-icon" />{" "}
						{event.summary}
					</p>
						...
						<div className="modal-tabs">
							<button
								className={`tab-button ${
									isTabActive("description") ? "active" : ""
								}`}
							...
							{event.photos && event.photos.length > 0 && (
								<button
									className={`tab-button ${
										isTabActive("photos") ? "active" : ""
									}`}
									onClick={() => setActiveTab("photos")}
								>
									<Image size={18} className="tab-icon" />{" "}
									Photos
								</button>
							)}
							{event.sponsors && event.sponsors.length > 0 && (
								<button
									className={`tab-button ${
										isTabActive("sponsors") ? "active" : ""
									}`}
									onClick={() => setActiveTab("sponsors")}
								>
									<Handshake size={18} className="tab-icon" />{" "}
									Sponsors
								</button>
							)}
						</div>
								...
								<div className="modal-tab-section">
									{event.descriptionDetails ? (
										<>
											...
											{event.descriptionDetails.perks &&
												event.descriptionDetails.perks
													.length > 0 && (
													<div className="perks-section">
														...
														<ul className="perks-list">
															{event.descriptionDetails.perks.map(
																(
																	perk,
																	index
																) => (
											...
											<>
												<h4 className="speaker-sub-heading">
													<UserPlus
														...
														className="section-heading-icon"
													/>{" "}
													Assistant & Support Speakers
													🤝
												</h4>
												<div className="speakers-list">
													{assistantSpeakers.map(
														(speaker, index) => (
															<div
																key={index}
																className="speaker-item"
